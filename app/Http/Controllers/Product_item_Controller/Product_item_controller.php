...
namespace App\Http\Controllers\Product_item_Controller;
...
class Product_item_controller extends Controller
{
    public function addItem(Request $request)
    {
        ...
        return response()->json(["msg" => "Item added Successfully "], 200);
    }
    ...
    public function addItemCriteria(Request $request)
    {
        ...
        $criteria_list = $request->input('criteria_list');
        $product_item = Product_item::find($item_id);
        foreach ($criteria_list as $criteriaItems) {
            $criteriaItem =  $criteriaItems;
            $criteria = Criteria_base::find($criteriaItem["criteria_id"]);
            $product_item->CriteriaBase()
    ...
    public function updateItemCriteria(Request $request)
    {
        ...
        $item = Product_item::find($item_id);
    ...
    public function getItem(Request $request, $item_id)
    {
       // $supplier = AuthController::me();
        // $item_id = $request->input('item_id');
        $item = Product_item::with(['CriteriaBase','images','product'=>function ($q)
        {$q->with('brand:id,brand_name','category:id,category_name',/*'supplier'*/)
            ->get();}])->find($item_id);
        return response()->json($item);
    }

    public function getProductItemList(Request $request,$product_id)
    {
       // $supplier = AuthController::me();
        //$product_id = $request->input('product_id');
        $itemList = Product_base::with(['brand', 'category', 'items' => function ($query) {
    ...
    public function searchItem(Request $request)
    {
        ...
        if (is_numeric($key_word)) {
            $items = Product_item::where('item_barcode', $key_word)->get();
            if (!$items->isEmpty()) {
            ...
            $product_base = Product_base::with('items')->where('product_name', $key_word)->get();
            return response()->json($product_base, 200);
        }
    }
